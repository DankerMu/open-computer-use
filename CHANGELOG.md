...
- **Preview shell public prefix.** `OCU_PUBLIC_PREFIX` (default empty) prefixes
  ...
  Invalid prefixes fail import/startup. Empty prefix keeps the previous URLs.
  A nonempty prefix is an intermediate server slice; issue17 must land before
  prefixed SPA deployment.
