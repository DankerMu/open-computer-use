...
- 运行环境必须显式提供 `OCU_PRIVATE_NETWORK`、`OCU_PRIVATE_SUBNET`、`OCU_PRIVATE_GATEWAY`、`OCU_SANDBOX_NETWORK`、`OCU_SANDBOX_SUBNET`、`OCU_SANDBOX_GATEWAY`、`OCU_SANDBOX_EGRESS_ALLOW`、`OCU_SANDBOX_DNS`、`OCU_PROXY_PORT`、`OCU_PROXY_IMAGE`、`OCU_INTERNAL_TOKEN`、`OCU_WEBUI_ORIGIN`、`OCU_WEBUI_AUTH_URL` 和 `PUBLIC_BASE_URL`，以及既有应用和 provider 的必要变量。`OCU_SANDBOX_EGRESS_ALLOW` 未设置是配置错误；显式空值表示拒绝全部新的 sandbox 出站。`OCU_SANDBOX_DNS` 未设置是配置错误；显式空值通过容器本地 `127.0.0.11` 覆盖关闭外部 DNS 转发，而不是继承宿主机 nameserver。不要将 token 写在命令行、日志或仓库文件中。`scripts/bootstrap-test.sh` 现在要求操作员提供匹配 checkout 的完整 `SOURCE_SHA`、全部运行时镜像引用和 `OCU_WEBUI_ORIGIN`，并写入上述拓扑/鉴权变量、`PUBLIC_BASE_URL=${OCU_WEBUI_ORIGIN}/ocu`、内部 `OCU_WEBUI_AUTH_URL=http://open-webui:8080/api/v1/ocu/auth`、共享内部 token、`OCU_PUBLIC_PREFIX=/ocu` 与 `OCU_SANDBOX_NO_AUTOSTART=1`。备份固定库存仍由 #34 更新。真实 Compose 启动、168 小时数据保留证据留给 #36。
